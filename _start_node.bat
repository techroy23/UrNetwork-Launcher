...
if not defined URL (
  echo ERROR: No urnetwork-provider-*.tar.gz asset found.
  exit /b 1
)
...
powershell -NoProfile -Command ^
  "$tgz = 'cached\%FILENAME%';" ^
  "$tmp = 'cached\extracted_node';" ^
  "$src = Join-Path $tmp ('windows\' + '%ARCH%');" ^
  "if (Test-Path $tmp) { Remove-Item $tmp -Recurse -Force };" ^
  "mkdir $tmp | Out-Null;" ^
  "tar -xzf $tgz -C $tmp;" ^
  "if (-not (Test-Path (Join-Path $src 'provider.exe'))) { Write-Error 'provider.exe not found after extraction'; exit 1 };" ^
  "Get-ChildItem -Path $src -File | Where-Object { $_.Name -ne 'provider.exe' } | Move-Item -Destination '.' -Force;" ^
  "Move-Item -Path (Join-Path $src 'provider.exe') -Destination 'ur_provider.exe' -Force"
if errorlevel 1 (
    if exist cached\extracted_node rd /s /q cached\extracted_node
    echo ERROR: Extraction failed or provider.exe not found.
    exit /b 1
)
if not exist "ur_provider.exe" (
    if exist cached\extracted_node rd /s /q cached\extracted_node
    echo ERROR: ur_provider.exe was not created after extraction.
    exit /b 1
)
...
for /f "tokens=1,2 delims==" %%A in ('powershell -NoProfile -Command ^
  ...
  "  if ($section -eq 'NODE' -and $_ -match '=') { $_ }" ^
  "}"') do (
...
if not defined USER (
    echo ============================================================
    echo   ERROR: USER not set in ENV.ini [NODE] section
    echo ============================================================
    exit /b 1
)
...
if not defined PASS (
    echo ============================================================
    echo   ERROR: PASS not set in ENV.ini [NODE] section
    echo ============================================================
    exit /b 1
)
...
echo ============================================================
ur_provider auth-provide --user_auth="%USER%" --password="%PASS%" -f
